...
#' @param query the required query. See [https://help.shortcut.com/hc/en-us/articles/360000046646-Search-Operators](https://help.shortcut.com/hc/en-us/articles/360000046646-Search-Operators) for options and the difference between story search operators and general search operators
...
#' \dontrun{
#' # Search for the term "foo" anywhere in stories
#' search_endpoint(endpoint = "stories", query = "foo")
#'
#' # Search for epics that contain specified text in name or description
#' # Note - no whitespace around the ":"
#' search_endpoint(endpoint = "epics", query = "title:foo")
#'
#' # Combine search terms with "AND" ("OR" combinations are not supported by the API V2)
#' # Note the whitespace between each filter
#' search_endpoint(endpoint = "stories", query = "epic:foo is:done type:bug")
#' }
#'
#' @export
